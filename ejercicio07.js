...
function genreMusicClassifier(songs) {
  const classifiedSongs = {}
  for (let i = 0; i < songs.length; i++) {
    const song = songs[i]
    const genre = song.genre
    if (classifiedSongs[genre]) {
      classifiedSongs[genre].push(song.title)
    } else {
      classifiedSongs[genre] = [song.title]
    }
  }
...
console.log(genreMusicClassifier(tracks))
