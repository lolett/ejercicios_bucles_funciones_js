...
function songCounterLength(list) {
  let lengthCounter = 0
  for (let i = 0; i < list.length; i++) {
    const song = list[i]
    if (song.genre === 'Rock') {
      lengthCounter = lengthCounter + song.duration
    }
  }

  return 'Minutos totales de rock: ' + lengthCounter
}

console.log(songCounterLength(albums))
